"""
Google Gemini AI Client for stock analysis service (API 방식)
"""
...
class GeminiAPIClient:
    """Google Gemini AI 클라이언트 (API 방식)"""
    
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, model: str = "gemini-2.0-flash-exp"):
    ...
    async def generate_response(self, prompt: str, **kwargs) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"🔗 Gemini API 호출: {url}")
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    logger.warning("⚠️ Gemini 응답에 content가 없습니다.")
                    return None
                else:
